# content-node â content transport (D2 v0a): fetch-by-hash over tcp
...
## v0a â GREEN (fetch-by-hash between two boxes, live)
...
Proven end-to-end under theater@00b0bf93 â the requested hash matched `sha256sum` of the
seed, and the client verified it after a real tcp hop:
```
[server] listening 127.0.0.1:9801, holds 1102d753â¦b413
[client] REQ_GET 1102d753â¦b413 -> 127.0.0.1:9801
[server] served BLOB 1102d753â¦b413 (34 bytes)
[client] BLOB 1102d753â¦b413 verified (34 bytes)   â   content-node-fetch-passed
```
...
SHA-256-verified against its hash on receipt (drop on mismatch) â content is immutable, so
...
tiny `role=â¦;listen=â¦;seed=â¦` / `role=client;peer=â¦;expect=<64-hex sha256>` string). Spawn
...
## v0b â GREEN (replicate-on-put to a holder roster, live)

A `role=primary` node puts a blob and PUSHes it to a comma-separated `holders` roster
(RF = roster size; content is immutable so replication needs no coordination â just copy).
...
(not the primary) and verifies it â so the holder served content it only obtained via
...
- liveness-aware quorum (count only holders that answer) â all content-layer-owned.
